...
class Xapi {
  ...
  asyncMiddleware(fn) {
    return (req, res, next) => {
      Promise.resolve(fn(req, res, next)).then(body => {
         if (body) {
           switch (req.query.format || req.get('Accept')) {
             case "application/xml":
             case "xml":
               var parser = new Parser({
                 format: true,
                 arrayMode: true,
                 cdataTagName: "article",
               });
               var results = [];
               body.forEach(async function(result) {
                 if (result && result.hasOwnProperty('article')) {
                   result['article'] = result['article'] ? {'article': result['article']} : result['article']
                 }
                 results.push(result)
              })
               if (res.headersSent) {
